...
const AnalysisReport = ({ result }) => {
  ...
  return (
    <Paper sx={{ p: 3, backgroundColor: '#2a2a2a', mt: 2, border: '1px solid rgba(255,255,255,0.23)' }}>
      ...
      {/* ... Summary, Steps Taken, and Recommendations sections ... */}
      <Box sx={{ my: 3 }}>
...
function ForecastPage() {
  ...
  useEffect(() => {
    if (selectedDataset) {
      setDataPreview(null);
      fetch(`http://127.0.0.1:8000/preview/${selectedDataset.filename}`)
        .then(res => res.json())
  ...
  const handleFollowUpSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:8000/follow-up', { method: 'POST', body: formData });
      if (!response.ok) { const err = await response.json(); throw new Error(err.detail); }
      const data = await response.json();
    ...
    <Box sx={{ padding: { xs: 2, sm: 4 }, color: 'white' }}>
      ...
      <Paper sx={{ p: { xs: 2, sm: 3 }, backgroundColor: '#1E1E1E', color: 'white' }}>
        ...
        <Box sx={{ marginTop: 4 }}>
          ...
          {isLoading ? (<Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100px' }}><CircularProgress /></Box>) : (analysisResult && <AnalysisReport result={analysisResult} />)}
